...
for i in $(seq 1 $total); do
	...
	CMplot(df, plot.type = "m", LOG10=TRUE, cex=0.4, threshold=9.18E-6, threshold.lty=2, threshold.col="black",  amplify=TRUE, signal.col="red", signal.cex=1,width=14, bin.size=1e6, chr.den.col=c("darkgreen", "yellow", "red"), verbose=TRUE, height=5, file="pdf", memo="$name.assoc.linear")
	EOF
